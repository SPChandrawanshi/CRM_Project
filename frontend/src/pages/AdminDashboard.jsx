...
const AdminDashboard = () => {
    ...
    const kpis = [
        { title: 'Global WhatsApp Nodes', value: dashboard.connectedWhatsapp, subValue: 'Active', icon: Phone, color: 'bg-emerald-50 text-emerald-600' },
        { title: 'Social Distribution', value: dashboard.facebookPages, subValue: 'Connected', icon: Facebook, color: 'bg-blue-50 text-blue-600' },
        { title: 'Network Lead Influx', value: dashboard.websiteLeads, subValue: 'Total', icon: MousePointer2, color: 'bg-indigo-50 text-indigo-600' },
    ]
        ...
        <div className="space-y-10 pb-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-5xl font-black text-slate-900 tracking-tighter uppercase">Admin Console</h1>
                ...
                <div className="flex items-center gap-4">
                    <button
                        ...
                        className="h-14 w-14 bg-white border border-slate-200 rounded-2xl flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:border-indigo-100 hover:shadow-xl hover:shadow-indigo-500/5 transition-all active:scale-95 group"
                    >
                        <RefreshCcw size={20} className={cn("transition-transform duration-700", isRefetching ? "animate-spin text-indigo-500" : "group-hover:rotate-180")} />
                    </button>
                    <button 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                <div className="lg:col-span-8 bg-white rounded-[2rem] border border-slate-200/60 shadow-sm overflow-hidden flex flex-col">
                    <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                        <div className="flex items-center gap-4">
                            <div className="h-10 w-10 bg-white rounded-xl border border-slate-100 shadow-sm flex items-center justify-center text-indigo-500">
                                <Shield size={18} />
                            </div>
                            <div>
                                <h3 className="font-black text-slate-900 text-xs uppercase tracking-[0.2em]">Matrix Distribution Preview</h3>
                                <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest leading-none">Autonomous assignment protocol validation</p>
                            </div>
                        </div>
                        <span className="badge badge-info bg-indigo-500/10 border-none text-indigo-600">LIVE FEED</span>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50/50">
                                    ...
                                    <th className="px-8 py-5">Core Target</th>
                                ...
                                {dashboard.routingPreview.map((rule, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50/30 transition-all group">
                                        <td className="px-8 py-8">
                                            <div className="flex items-center gap-4">
                                                <div className="h-3 w-3 rounded-full bg-slate-200 group-hover:bg-indigo-500 transition-colors shadow-inner" />
                                                <span className="font-black text-xs text-slate-900 uppercase tracking-tight leading-none">{rule.country}</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-8">
                                            <div className="flex flex-col gap-1">
                                                <span className="text-[11px] font-black text-slate-700 uppercase tracking-tight leading-none">{rule.team}</span>
                                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">{rule.counselor}</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-8">
                                            <span className="text-[9px] font-black text-indigo-600 uppercase tracking-widest bg-indigo-500/10 px-3 py-1.5 rounded-full border border-indigo-100/50">
                                                {rule.type}
                                            </span>
                                        </td>
                                        <td className="px-8 py-8">
                                            <div className="flex items-center gap-3">
                                                <div className={cn("h-2 w-2 rounded-full", rule.status === 'Active' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-slate-300')} />
                                                <span className={cn("text-[9px] font-black uppercase tracking-widest leading-none", rule.status === 'Active' ? 'text-emerald-600' : 'text-slate-400')}>
                                                    {rule.status}
                                                </span>
                                            </div>
                                        </td>
                ...
                <div className="lg:col-span-4 space-y-8">
                    <motion.div
                        className="bg-[#020617] rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl shadow-slate-900/20 border border-slate-800/50"
                    >
                        <div className="relative z-10">
                            <div className="flex items-center justify-between mb-12">
                                <div className="h-14 w-14 bg-indigo-500/10 rounded-2xl backdrop-blur-md border border-indigo-500/20 flex items-center justify-center">
                                    <BrainCircuit size={28} className="text-indigo-400" />
                                </div>
                                <div className="flex flex-col items-end gap-2">
                                    <div className="flex items-center gap-3">
                                        <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">System Pulse</span>
                                        <div className={cn("h-2 w-2 rounded-full", dashboard.aiStatus.enabled ? "bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.5)] animate-pulse" : "bg-rose-400")} />
                                    </div>
                                    <span className="text-[10px] font-black uppercase tracking-[0.1em] text-indigo-400 animate-pulse">{dashboard.aiStatus.enabled ? 'NEURAL ACTIVE' : 'SYSTEM IDLE'}</span>
                                </div>
                            </div>

                            <h3 className="text-3xl font-black uppercase tracking-tighter mb-3 leading-tight">Artificial<br />Intelligence</h3>
                            <p className="text-[10px] font-bold text-slate-500 leading-relaxed mb-10 uppercase tracking-[0.2em]">Autonomous lead validation and qualification layer v4.2</p>

                            <div className="p-6 bg-slate-900/50 rounded-2xl border border-slate-800/50 mb-8 backdrop-blur-md">
                                <div className="flex items-center justify-between">
                                    ...
                                    <button
                                        onClick={() => publishFlow.mutate({ threshold: 75, rules: [], scoringRules: [], flow: [], toggles: { aiEnabled: !dashboard.aiStatus.enabled } })}
                                        className={cn(
